...
# 把64x64的大块分成2x2布局的32x32虚拟块，tyz = (0, 2)
tyz, fi = s[B].split(fi, nparts=vthread)  # virtual thread split
txz, ni = s[B].split(ni, nparts=vthread)  # virtual thread split
# 把虚拟块分成8x8布局的4x4小块，每个小块交给一个线程， 
# tx,ty=(0, 8)    fi, ni=(0, 4)
ty, fi = s[B].split(fi, nparts=num_thread)
tx, ni = s[B].split(ni, nparts=num_thread)
# 这一步tyz txz可有可无，不影响最终代码
# 猜测是tyz 和txz的数值太小，自动循环展开了
# 具体需要再了解一下虚拟线程的意义和实现
s[B].reorder(bz, by, bx, tyz, txz, ty, tx, fi, ni)  
# s[B].reorder(bz, by, bx, ty, tx, tyz, txz, fi, ni)
...
s[BL].compute_at(s[B], tx)  # 线程级别写寄存器，每个线程要写4x4x4=64个B元素
yi, xi, fi, ni = s[BL].op.axis  # 分别是图像的高、宽、通道和样本数量
ry, rx, rc = s[BL].op.reduce_axis   # 每个元素都需要遍历计算输入通道和Kernel宽、高
rco, rci = s[BL].split(rc, factor=step) # 把通道按照划分为step步
s[BL].reorder(rco, ry, rx, rci, fi, ni) 

# Attach computation to iteration variables
# 在确定kernel坐标，以及通道起始位置后，进行共享内存协同加载，每次加载step个通道
