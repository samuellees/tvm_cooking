...
nchw_ho_wo_ksize_stride_pad_list = [
    [32, 1024, 7, 7, 1, 1, 7, 1, 0]
]
device = "cuda"
...
for i in range(len(nchw_ho_wo_ksize_stride_pad_list)):

    time_begin = time.time()
    N, C, H, W, HO, WO, ksize, stride, padding = nchw_ho_wo_ksize_stride_pad_list[i]
    func_name = "pool_avg_bwd_n" + str(N) + "_c" + str(C) + "_h" + str(H) + "_w" + str(W)  \
        + "_ho" + str(HO) + "_wo" + str(WO) + "_k" + str(ksize) + "_s" + str(stride)   \
        + "_p" + str(padding) + "_" + str(device)
