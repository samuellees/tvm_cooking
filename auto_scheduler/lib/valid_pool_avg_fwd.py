...
for i in range(len(nchw_ksize_stride_pad_list)):
    ...
    pool_torch = torch.nn.AvgPool2d((ksize, ksize), stride=stride, padding=padding)
    # np arrays
    data_np = data_torch.numpy()
    # torch result
    data_torch = data_torch.cuda()
    pool_torch = pool_torch.cuda()
    out_torch = pool_torch(data_torch)
    # np results
